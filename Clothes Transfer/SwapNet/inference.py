...
from PIL import Image  #to save the source image in texture stage
...
def _run_test_loop(model, dataset, webpage=None, iteration_post_hook: Callable = None):
    ...
    with tqdm(total=total, unit="img") as pbar:
        for i, data in enumerate(dataset):
            ...
            #to print the source images in the texture stage
            if not iteration_post_hook:
                p = str(image_paths[0]).split('warp/')[1].split('_to')[0]
                img = Image.open(f'/content/drive/My Drive/Clothes Transfer/SwapNet/data/deep_fashion/texture/MEN/Tees_Tanks/id_00000045/{p}.jpg')
                img.save(str(image_paths[0])[2:].split('warp/')[0]+'texture/images/'+p+'_source.png')
            ################################################
            if webpage:
